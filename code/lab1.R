...
# em uma imagem no formato png
   ...
   p<-ggplot(data = tabela_por_estado) + 
     geom_bar(mapping = aes(x = `Estado de nascimento (abreviado)`, y = Percentual), 
              colour = "white", fill = "orange",stat="identity")
     # geom_text(aes(y=Percentual,label=Percentual,vjust=-0.3,size=3.5))+
    #geom_text(aes(vjust=-0.3,size=3.5))
   
   #g<- ggplot(data = tabela_por_estado) + 
   #   geom_bar(mapping = aes(x=`Estado de nascimento (abreviado)`, y=Percentual, color = "white", fill = "orange"), 
   #            stat = "identity")
   ...
   #g + geom_text(aes(y=Percentual, label=Percentual))
   #g
   
   p<-pdf(file = 'nome.pdf', family = 'Times')
   print(p)
   dev.off()
